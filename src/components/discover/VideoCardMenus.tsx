...
export function buildContextMenuItems(
  ...
  onSave: ((video: VideoWithOutlier) => void) | undefined,
  openBoost: () => void,
): ContextMenuItem[] {
  return [
    {
    ...
    },
    {
      ...
      onClick: () => {
        const lang = detectLanguage(video.title + " " + (video.description || ""));
        blockLanguage(lang);
    ...
    },
    {
      ...
      onClick: () => {
        if (video.channelId) {
          blockCreator(video.channelId);
          toast.success(`Hid creator: ${video.channelTitle}`);
          window.dispatchEvent(new Event("tubeforge-blocklist-updated"));
        }
      },
    },
...
export function buildBoostItems(
  video: VideoWithOutlier,
  onChatOpen: ((video: VideoWithOutlier, initialPrompt?: string) => void) | undefined,
  closeBoost: () => void,
): BoostItem[] {
  return [
    {
    ...
    },
    {
      ...
      onClick: () => {
        const prompt = `Generate headline variations for: "${video.title}"`;
        if (onChatOpen) {
          onChatOpen(video, prompt);
        }
        closeBoost();
      },
    },
    {
      ...
      onClick: () => {
        const prompt = `Based on this video, break it into 5 distinct short-post ideas I could publish. For each idea, give a short hook line and a 1-2 sentence outline. Use the video's topic, hook, and structure as the foundation.`;
        if (onChatOpen) {
          onChatOpen(video, prompt);
        }
        closeBoost();
    ...
    },
    {
      ...
      onClick: () => {
        const prompt = `Reverse engineer this video. Analyze why it works — the hook, structure, pacing, and angle — and explain the method so I can apply the same approach to write my own post on this topic.`;
        if (onChatOpen) {
          onChatOpen(video, prompt);
        }
        closeBoost();
