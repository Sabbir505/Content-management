...
export async function searchHackerNewsByTopic(topic: string, limit = 15): Promise<ContentSearchResult> {
  return rateLimiter.executeWithRetry("hackernews", async () => {
    ...
    const searchUrl = `https://hn.algolia.com/api/v1/search?query=${encodeURIComponent(topic)}&tags=story&hitsPerPage=${limit}`;

    const response = await proxyFetch(searchUrl);
    const data = await response.json();

    const items: ContentItem[] = data.hits.map((hit: any) => ({
      id: `hn-${hit.objectID}`,
